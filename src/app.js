const { incrementQty, decrementQty, recalculateSubtotal } = require('./helpers.js')
...
confirmCodeButton.addEventListener('click', () => {
  if (codeText.value === 'KODE27') {
    discount = 27
    subtotalText.textContent = `Rp. ${recalculateSubtotal( priceInput.value, qtyInput.value, discount)}`
  } else {
    discount = 0
    subtotalText.textContent = `Rp. ${recalculateSubtotal( priceInput.value, qtyInput.value, discount)}`
  }
